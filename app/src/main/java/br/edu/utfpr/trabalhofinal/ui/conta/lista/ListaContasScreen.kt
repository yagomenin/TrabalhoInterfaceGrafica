...
private fun List(
    ...
    LazyColumn(modifier = modifier) {
        items(contas) { conta ->
            val tipoConta = TipoContaEnum.DESPESA
            val data = conta.data.formatar()
            val descricao = conta.descricao
            val valor = geraValorCorreto(conta, tipoConta)
            val icone: ImageVector = geraIcons(conta, tipoConta)
            val corIcone: Color = geraIconsColor(conta, tipoConta)
            ListItem(
                ...
                headlineContent = { Column {
                    ...
                    Row (
                        ...
                        Text(text = data)
                        Text(text = valor,
                            color = if (conta.tipo == tipoConta) Color(0xFFCF5355) else Color(0xFF00984E))
                        }
                    }
                },
            )
...
fun Totalizador(
    ...
    Row(
        ...
        Text(
            ...
            textAlign = TextAlign.End,
            text = if (titulo == "Saldo") valor.formatar() else " - ${valor.formatar()}",
            color = if (titulo == "Saldo") Color(0xFF00984E) else Color(0xFFCF5355)
        )
...
private fun geraIcons(conta: Conta, tipoConta: TipoContaEnum): ImageVector {
    if (conta.tipo == tipoConta && conta.paga) {
        return Icons.Filled.ThumbUp
    } else if (conta.tipo == tipoConta) {
...
private fun geraIconsColor(conta: Conta, tipoConta: TipoContaEnum): Color {
    if(conta.tipo == tipoConta) {
...
private fun geraValorCorreto(conta: Conta, tipoConta: TipoContaEnum): String {
    if (conta.tipo == tipoConta) {
        return "- R$ ${conta.valor}"
    } else {
        return "+ R$ ${conta.valor}"
    }
}
